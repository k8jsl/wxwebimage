<?php
		...
		class Tools
		{
		   ...
		   var $VERSION = "WxWeb Image Tools.php 3.16<br />";
		   ...
		   function do_title($title, $credit, $image)
		   {
		      ...
		      $black = self::color($image, '000000');
		      $crcolor = self::color($image, $creditcolor);
		      $ticolor = self::color($image, $titlecolor);

		      // _text_width_height($text,$fontPath,$fontFile,$fontSize,$fontAngle)
		      list($w, $h) = self::_text_width_height($credit, $fontpath, $creditfont, $creditfontsize, '0');

		      list($creditx, $credity) = self::_text_alignment(imagesx($image), imagesy($image), imagesx($image) - 8, $scredity, $w, $h, 'RIGHT');
		   ...
		   function do_text($image, $text, $x, $y, $chest)
		   {
		      ...
		      $fhex = self::color($image, $chest['ttfcolor']);
		      ...
		      if (strtoupper($halign) == 'SPACEWRAP')
		      {
		         ...
		         $image = self::DoSpacewrap($image, $lines, $x, $y, $chest);
		         return $image;
		         break;
		      }


		      list($text_w, $text_h) = self::_text_width_height($text, $chest['ttfpath'], $chest['ttffont'], $chest['ttfpt'], $chest['ttfangle']);


		      list($ux, $uy) = self::_text_alignment(imagesx($image), imagesy($image), $x, $y, $text_w, $text_h, $halign);
		   ...
		   function DoSpacewrap($image, $lines, $x, $y, $chest)
		   {
		      ...
		      foreach ($lines as $text)
		      {
		         ...
		         list($ux, $uy) = self::_text_alignment(imagesx($image), imagesy($image), $x, $y, $text_w, $text_h, 'CENTER');
		   ...
		   function do_nodata($image)
		   {
		      ...
		      $image = self::do_text($image, $label, $xx, $yy, $chest);
		   ...
		   */
		   ...
		   function color($image, $color)
		   {
		      ...
		      $resource = imagecolorallocate($image, $r, $g, $b);
